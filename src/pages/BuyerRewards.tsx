...
import { BUYER_LEVELS } from '../lib/gamification';
import { cn } from '../lib/utils';

export default function BuyerRewards() {
  const { stats } = useGamification('buyer');
  const [selectedTab, setSelectedTab] = useState(0);

  // Get next 3 levels
  const currentLevelIndex = BUYER_LEVELS.findIndex(level => level.name === stats.level.name);
  const nextLevels = BUYER_LEVELS.slice(currentLevelIndex + 1, currentLevelIndex + 4);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Suivez vos points, niveaux et récompenses
          </p>
        </div>
